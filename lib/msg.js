...
const sms = (conn, m, store) => {
    ...
    if (m.message) {
        ...
        if (m.quoted) {
            ...
            // View-once wrapper ko unwrap karo agar WhatsApp ne isko wrapper ke andar bheja hai
            let isViewOnce = false
            if (['viewOnceMessage', 'viewOnceMessageV2', 'viewOnceMessageV2Extension'].includes(type)) {
                isViewOnce = true
                const innerType = getContentType(m.quoted.message)
                type = innerType
            ...
            m.quoted.mtype = type
            // Native viewOnce flag (jo WhatsApp khud imageMessage/videoMessage ke andar bhejta hai) preserve karo,
            // wrapper-detected flag ke saath OR karo taake overwrite na ho
            m.quoted.viewOnce = isViewOnce || !!m.quoted.viewOnce
            ...
            if (m.quoted && m.quoted.chat) {
                let vM = m.quoted.fakeObj = M.fromObject({
                    key: {
                        remoteJid: m.quoted.chat || m.chat,
                        fromMe: m.quoted.fromMe || false,
                        id: m.quoted.id || 'unknown'
                    },
                    message: quoted || {},
                    ...(m.isGroup ? { participant: m.quoted.sender || m.sender } : {})
                })
                ...
                // downloadContentFromMessage direct use karo — conn.downloadMediaMessage exist nahi karta Baileys mein
                m.quoted.download = async () => {
                    const dtype = m.quoted.mtype.replace('Message', '') // image / video / audio / sticker / document
                    const stream = await downloadContentFromMessage(m.quoted, dtype)
                    let buffer = Buffer.from([])
                    for await (const chunk of stream) {
                        buffer = Buffer.concat([buffer, chunk])
                    }
                ...
                console.log('⚠️ Quoted message data missing, skipping fakeObj creation');
                m.quoted = m.quoted || {}
    ...
    if (m.msg && m.msg.url) {
        m.download = async () => {
            const dtype = m.mtype.replace('Message', '')
            const stream = await downloadContentFromMessage(m.msg, dtype)
            let buffer = Buffer.from([])
            for await (const chunk of stream) {
                buffer = Buffer.concat([buffer, chunk])
            }
